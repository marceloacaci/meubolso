...
async function atualizarPerfisInfo() {
  try {
    const r = await window.api.perfilListar();
    if (r && r.perfis) perfisInfo = { ativo: r.ativo, perfis: r.perfis };
  } catch (_) {}
  window.__perfisInfo = perfisInfo;
  ...
  if (el) {
    ...
    if (ativo) {
      const busto =
        '<svg class="usuario-ico" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="8" r="4"></circle><path d="M4 20c0-4 4-6 8-6s8 2 8 6"></path></svg>';
      el.innerHTML =
        busto +
        '<span class="usuario-nome">' +
        t('perfil.usuario') +
        ': ' +
        escapeHtml(ativo.nome) +
        '</span>';
      el.style.display = '';
    } else {
      el.innerHTML = '';
...
const DICAS = [
  {
    pt: 'Crie uma reserva de emergência equivalente a 3 a 6 meses dos seus gastos.',
    en: 'Build an emergency fund worth 3 to 6 months of your expenses.',
    es: 'Crea un fondo de emergencia equivante a 3-6 meses de tus gastos.',
  },
  {
    pt: 'Pague as dívidas com os maiores juros primeiro (método avalanche).',
    en: 'Pay the highest-interest debts first (avalanche method).',
    es: 'Paga las dudas con mayores intereses primero (método avalancha).',
  },
  {
    pt: 'Anote todos os gastos por 30 dias para descobrir para onde vai o dinheiro.',
    en: 'Track every expense for 30 days to see where your money goes.',
    es: 'Anota todos los gastos por 30 días para ver a dónde va el dinero.',
  },
  {
    pt: 'Regra 50/30/20: 50% necessidades, 30% desejos, 20% poupança e dívidas.',
    en: 'Rule 50/30/20: 50% needs, 30% wants, 20% savings and debt.',
    es: 'Regla 50/30/20: 50% necesidades, 30% deseos, 20% ahorro y dudas.',
  },
  {
    pt: 'Evite pagar só o mínimo do cartão de crédito: os juros são altíssimos.',
    en: 'Avoid paying only the credit card minimum: the interest is very high.',
    es: 'Evita pagar solo el mínimo de la tarjeta: los intereses son altísimos.',
  },
  {
    pt: 'Negocie prazos e taxas com credores antes de deixar a dívida vencer.',
    en: 'Negotiate terms and rates with creditors before the debt is due.',
    es: 'Negocia plazos y tasas con acreedores antes de que venza la duda.',
  },
  {
    pt: 'Use dinheiro ou débito para compras de impulso e evite surpresas na fatura.',
    en: 'Use cash or debit for impulse buys and avoid bill surprises.',
    es: 'Usa efectivo o débito para compras por impulso y evita sorpresas en el resumen.',
  },
  {
    pt: 'Reveja assinaturas recorrentes (streaming, apps) pelo menos 1x por ano.',
    en: 'Review recurring subscriptions (streaming, apps) at least once a year.',
    es: 'Revisa suscripciones recorrentes (streaming, apps) al menos 1 vez al año.',
  },
  {
    pt: 'Estabeleça metas financeiras claras e com prazo: torna poupar mais fácil.',
    en: 'Set clear, time-bound financial goals: it makes saving easier.',
    es: 'Establece metas financieras claras y con plazo: ahorrar se vuelve más fácil.',
  },
  {
    pt: 'Automatize a poupança: transfira um valor todo mês no dia do salário.',
    en: 'Automate savings: transfer an amount every month on payday.',
    es: 'Automatiza el ahorro: transfiere un monto cada mes el día del sueldo.',
  },
  {
    pt: 'Compare preços antes de comprar: a diferença pode ser de 30% ou mais.',
    en: 'Compare prices before buying: the difference can be 30% or more.',
    es: 'Compara precios antes de comprar: la diferencia puede ser del 30% o más.',
  },
  {
    pt: 'Não misture dinheiro de lazer com dinheiro de contas fixas.',
    en: 'Do not mix leisure money with fixed-bills money.',
    es: 'No mezzcles dinero de ocio con dinero de cuentas fijas.',
  },
  {
    pt: 'Fique atento ao Custo Efetivo Total (CET) de empréstimos e financiamentos.',
    en: 'Watch the Total Effective Cost (CET) of loans and financing.',
    es: 'Presta atención al Costo Efetivo Total (CET) de préstamos y financiaciones.',
  },
  {
    pt: 'Prefira quitar à vista quando o desconto supera a rentabilidade da poupança.',
    en: 'Prefer paying in full when the discount beats savings returns.',
    es: 'Prefiere pagar al contado cuando el descuento supera la rentabilidad del ahorro.',
  },
  {
    pt: 'Tenha um orçamento mensal escrito — quem não mede, não controla.',
    en: 'Keep a written monthly budget — what is not measured is not controlled.',
    es: 'Ten un presupuesto mensual por escrito — quien no mide, no controla.',
  },
  {
    pt: 'Reduza o uso de cheques especiais: o juro diário é um dos maiores do mercado.',
    en: 'Cut overdraft use: the daily interest is among the highest.',
    es: 'Reduce el uso de cheques especiales: el interés diario es de los más altos.',
  },
  {
    pt: 'Compre em atacado itens de consumo contínuo para economizar no longo prazo.',
    en: 'Buy staples in bulk to save over the long run.',
    es: 'Compra al por mayor artículos de consumo continuo para ahorrar a largo plazo.',
  },
  {
    pt: 'Revise seu plano de celular: planos antigos costumam ser mais caros.',
    en: 'Review your phone plan: old plans tend to be more expensive.',
    es: 'Revisa tu plan de celular: los planos antiguos suelen ser más caros.',
  },
  {
    pt: 'Use a técnica de esperar 24h antes de compras não essenciais.',
    en: 'Use the 24-hour wait rule before non-essential purchases.',
    es: 'Usa la técnica de esperar 24h antes de compras no esenciais.',
  },
  {
    pt: 'Mantenha contas separadas: corrente para gastos e poupança para objetivos.',
    en: 'Keep separate accounts: checking for spending, savings for goals.',
    es: 'Manten cuentas separadas: corriente para gastos y ahorro para objetivos.',
  },
  {
    pt: 'Aumente a renda negociando um aumento ou uma fonte extra de trabalho.',
    en: 'Boost income by negotiating a raise or a side job.',
    es: 'Aumenta el ingreso negociando un aumento o un trabajo extra.',
  },
  {
    pt: 'Pague contas em dia para não desperdiçar dinheiro com multas e juros.',
    en: 'Pay bills on time to avoid wasting money on fees and interest.',
    es: 'Paga las cuentas a tiempo para no desperdiciar dinero en multas e intereses.',
  },
  {
    pt: 'Evite financiar bens que desvalorizam (carro, eletrônicos) por muito tempo.',
    en: 'Avoid financing depreciating assets (car, electronics) for long periods.',
    es: 'Evita financiar bienes que se devalúan (coche, electrónicos) por mucho tiempo.',
  },
  {
    pt: 'Faça um fundo para impostos e despesas anuais, não só mensais.',
    en: 'Create a fund for taxes and yearly expenses, not just monthly.',
    es: 'Crea un fundo para impuestos y gastos anuales, no solo mensuales.',
  },
  {
    pt: 'Não use o limite do cartão como se fosse renda disponível.',
    en: 'Do not treat your card limit as available income.',
    es: 'No uses el límite de la tarjeta como si fuera ingreso disponible.',
  },
  {
    pt: 'Priorize quitar dívidas antes de investir em renda variável.',
    en: 'Prioritize paying off debt before investing in variable income.',
    es: 'Prioriza pagar dudas antes de invertir en renta variable.',
  },
  {
    pt: 'Leia o contrato antes de assinar: entenda IOF, seguros e taxas.',
    en: 'Read the contract before signing: understand IOF, insurance and fees.',
    es: 'Le el contrato antes de firmar: entiende IOF, seguros y tasas.',
  },
  {
    pt: 'Use aplicativos de controle financeiro para visualizar sua evolução.',
    en: 'Use finance apps to visualize your progress.',
    es: 'Usa aplicaciones de control financiero para visualizar tu evolución.',
  },
  {
    pt: 'Corrija o hábito de comprar por ansiedade ou tédio, não por necessidade.',
    en: 'Fix the habit of buying out of anxiety or boredom, not need.',
    es: 'Corrige el hábito de comprar por ansiedad o tedio, no por necesidad.',
  },
  {
    pt: 'Renegocie dívidas em atraso: credores preferem receber parcelado a nada.',
    en: 'Renegotiate overdue debts: creditors prefer installments to nothing.',
    es: 'Renegocia dudas en mora: los acreedores prefieren cuotas a nada.',
  },
  {
    pt: 'Invista em educação financeira — é o melhor retorno a longo prazo.',
    en: 'Invest in financial education — best long-term return.',
    es: 'Invierte en educación financiera — es el mejor retorno a largo plazo.',
  },
  {
    pt: 'Evite ser fiador de terceiros: a dívida pode virar sua sem aviso.',
    en: 'Avoid co-signing for others: the debt may become yours.',
    es: 'Evita ser fiador de terceros: la duda puede volverse tuya sin aviso.',
  },
  {
    pt: 'Compre bens usados de qualidade: economiza até 50% com mesma utilidade.',
    en: 'Buy quality used goods: save up to 50% with same utility.',
    es: 'Compra bienes usados de calidad: ahorra hasta 50% con la misma utilidad.',
  },
  {
    pt: 'Não tome empréstimo para pagar outra dívida sem plano de saída.',
    en: 'Do not borrow to pay another debt without an exit plan.',
    es: 'No pidas préstamo para pagar otra duda sin plan de salida.',
  },
  {
    pt: 'Monitore seu score de crédito: influencia juros de futuros contratos.',
    en: 'Monitor your credit score: it affects future loan rates.',
    es: 'Monitoriza tu score de crédito: influye en los intereses de futuros contratos.',
  },
  {
    pt: 'Separe um valor fixo para lazer no orçamento — restrição total não dura.',
    en: 'Set a fixed fun-money amount in your budget — total restriction fails.',
    es: 'Separa un monto fijo para ocio en el presupuesto — la restricción total no dura.',
  },
  {
    pt: 'Use listas de compras para não cair em promoções não planejadas.',
    en: 'Use shopping lists to avoid unplanned promotions.',
    es: 'Usa listas de compras para no caer en promociones no planeadas.',
  },
  {
    pt: 'Aprenda a diferença entre ativo (gera renda) e passivo (custa dinheiro).',
    en: 'Learn the difference between assets (earn) and liabilities (cost).',
    es: 'Aprende la diferencia entre activo (genera ingreso) y pasivo (cuesta dinero).',
  },
  {
    pt: 'Faça manutenção preventiva: evita reparos caros no futuro.',
    en: 'Do preventive maintenance: avoids costly future repairs.',
    es: 'Haz mantenimiento preventivo: evita reparaciones caras en el futuro.',
  },
  {
    pt: 'Negocie descontos à vista em serviços e consultorias.',
    en: 'Negotiate cash discounts on services and consulting.',
    es: 'Negocia descuentos al contado en servicios y consultorías.',
  },
  {
    pt: 'Não saia de casa sem um teto de gasto definido para o dia.',
    en: 'Do not leave home without a daily spending cap.',
    es: 'No salgas de casa sin un tope de gasto definido para el día.',
  },
  {
    pt: 'Converse com a família sobre dinheiro: metas em comum têm mais chance.',
    en: 'Talk with family about money: shared goals stand a better chance.',
    es: 'Habla con la familia sobre dinero: las metas en común tienen más posibilidades.',
  },
  {
    pt: 'Evite o "efeito treino": não gaste mais só porque ganhou bônus ou 13º.',
    en: 'Avoid the "treat effect": do not spend more just because of a bonus.',
    es: 'Evita el "efecto premio": no gastes más solo por un bónus o el 13º.',
  },
  {
    pt: 'Compare o custo por uso de bens antes de adquirir algo caro.',
    en: 'Compare cost-per-use of items before buying something pricey.',
    es: 'Compara el costo por uso de bienes antes de adquirir algo caro.',
  },
  {
    pt: 'Mantenha documentos e boletos organizados para pagar no prazo.',
    en: 'Keep documents and bills organized to pay on time.',
    es: 'Manten documentos y boletos organizados para pagar a tiempo.',
  },
  {
    pt: 'Use a regra dos 30 dias para compras grandes e evite arrependimento.',
    en: 'Use the 30-day rule for big purchases and avoid regret.',
    es: 'Usa la regla de 30 días para compras grandes y evita arrepentimiento.',
  },
  {
    pt: 'Reveja seu seguro: cobertura adequada evita perdas financeiras enormes.',
    en: 'Review your insurance: right coverage avoids huge financial losses.',
    es: 'Revisa tu seguro: cobertura adecuada evita pérdidas financieras enormes.',
  },
  {
    pt: 'Lembre-se: liberdade financeira é construída com pequenas decisões diárias.',
    en: 'Remember: financial freedom is built with small daily choices.',
    es: 'Recuerda: la libertad financiera se construe con pequeñas decisones diarias.',
  },
  {
    pt: 'Comece cedo: o tempo é o maior aliado dos juros compostos.',
    en: 'Start early: time is the biggest ally of compound interest.',
    es: 'Empieza temprano: el tiempo es el mayor aliado de los intereses compuestos.',
  },
  {
    pt: 'Pague a si mesmo primeiro: reserve antes de gastar.',
    en: 'Pay yourself first: set aside before spending.',
    es: 'Páguese a sí mismo primero: reserva antes de gastar.',
  },
  {
    pt: 'Evite comprar para "aparentar" — status não paga contas.',
    en: 'Avoid buying to "show off" — status does not pay bills.',
    es: 'Evita comprar para "aparentar" — el estatus no paga cuentas.',
  },
  {
    pt: 'Tenha 3 contas: gastos, emergência e objetivos de longo prazo.',
    en: 'Use 3 accounts: spending, emergency and long-term goals.',
    es: 'Ten 3 cuentas: gastos, emergencia y objetivos a largo plazo.',
  },
  {
    pt: 'Reveja sua vida de assinaturas: cancele o que não usa.',
    en: 'Audit your subscriptions: cancel what you do not use.',
    es: 'Audita tus suscripciones: cancela lo que no usas.',
  },
  {
    pt: 'Compre qualidade durável: mais caro hoje, mais barato no total.',
    en: 'Buy durable quality: pricier now, cheaper overall.',
    es: 'Compra calidad duradera: más caro hoy, más barato en total.',
  },
  {
    pt: 'Estipule um limite semanal de "prazer" e respeite-o.',
    en: 'Set a weekly "fun" limit and stick to it.',
    es: 'Estipula un límite semanal de "placer" y respétalo.',
  },
  {
    pt: 'Não compare sua jornada com o fim de outra pessoa.',
    en: 'Do not compare your journey to someone elses finish.',
    es: 'No compares tu recorrido con el final de otra persona.',
  },
  {
    pt: 'Use a regra 24h para compras acima de 10% da sua renda.',
    en: 'Use the 24h rule for buys above 10% of your income.',
    es: 'Usa la regla 24h para compras sobre el 10% de tu ingreso.',
  },
  {
    pt: 'Automatize contas fixas para nunca esquecer um vencimento.',
    en: 'Automate fixed bills so you never miss a due date.',
    es: 'Automatiza cuentas fijas para nunca olvidar un vencimiento.',
  },
  {
    pt: 'Mantenha um "fundo do carro" para manutenção e impostos.',
    en: 'Keep a "car fund" for upkeep and taxes.',
    es: 'Manten un "fondo del coche" para mantenimiento e impuestos.',
  },
  {
    pt: 'Priorize aposentadoria: contribua mesmo que pouco.',
    en: 'Prioritize retirement: contribute even if small.',
    es: 'Prioriza la jubilación: aporta aunque sea poco.',
  },
  {
    pt: 'Evite a "economia de pontos": não gaste só para acumular.',
    en: 'Avoid "points economics": do not spend just to earn.',
    es: 'Evita la "economía de puntos": no gastes solo para acumular.',
  },
  {
    pt: 'Negocie sua fatura de cartão se estiver com dificuldade.',
    en: 'Negotiate your card bill if you are struggling.',
    es: 'Negocia tu resumen de tarjeta si estás en dificultad.',
  },
  {
    pt: 'Use cupons e cashback, mas só em o que já compraria.',
    en: 'Use coupons and cashback, but only on what you would buy.',
    es: 'Usa cupones y cashback, pero solo en lo que ya comprarías.',
  },
  {
    pt: 'Não deixe dinheiro parado em contas sem rendimento.',
    en: 'Do not leave money idle in non-interest accounts.',
    es: 'No dejes dinero ocioso en cuentas sin rendimiento.',
  },
  {
    pt: 'Crie um "orçamento de felicidade": gaste com experiências.',
    en: 'Create a "happiness budget": spend on experiences.',
    es: 'Crea un "presupuesto de felicidad": gasta en experiencias.',
  },
  {
    pt: 'Evite lojas de conveniência: o preço por unidade é maior.',
    en: 'Avoid convenience stores: unit price is higher.',
    es: 'Evita tiendas de conveniencia: el precio por unidad es mayor.',
  },
  {
    pt: 'Pague dívidas do tipo "revolving" antes das de baixo juros.',
    en: 'Pay revolving debts before low-interest ones.',
    es: 'Paga dudas "revolving" antes que las de bajo interés.',
  },
  {
    pt: 'Mantenha 3 a 6 meses de despesa em fundo de emergência.',
    en: 'Keep 3 to 6 months of expenses in an emergency fund.',
    es: 'Manten 3 a 6 meses de gastos en fundo de emergencia.',
  },
  {
    pt: 'Use a regra 50/15/35: 50% contas, 15% lazer, 35% futuro.',
    en: 'Use 50/15/35: 50% bills, 15% fun, 35% future.',
    es: 'Usa 50/15/35: 50% cuentas, 15% ocio, 35% futuro.',
  },
  {
    pt: 'Venda o que não usa: desapego gera caixa.',
    en: 'Sell what you do not use: letting go creates cash.',
    es: 'Vende lo que no usas: el desapego genera efectivo.',
  },
  {
    pt: 'Evite compras "em conta" se não precisar daquilo.',
    en: 'Avoid "cheap" buys if you do not need the item.',
    es: 'Evita compras "baratas" si no necesitas eso.',
  },
  {
    pt: 'Aprenda a ler um demonstrativo bancário.',
    en: 'Learn to read a bank statement.',
    es: 'Aprende a leer un estado de cuenta bancario.',
  },
  {
    pt: 'Estabeleça um "dia sem gastos" por semana.',
    en: 'Set a "no-spend day" each week.',
    es: 'Establece un "día sin gastos" por semana.',
  },
  {
    pt: 'Use a técnica de "envelopes" para controlar categorias.',
    en: 'Use the "envelope" method to control categories.',
    es: 'Usa la técnica de "sobres" para controlar categorías.',
  },
  {
    pt: 'Não financie o estilo de vida com dívida de consumo.',
    en: 'Do not finance your lifestyle with consumer debt.',
    es: 'No financies tu estilo de vida con duda de consumo.',
  },
  {
    pt: 'Recompense-se com metas atingidas, mas sem estourar.',
    en: 'Reward yourself on hits, but without overspending.',
    es: 'Premiate con metas logradas, pero sin excederte.',
  },
  {
    pt: 'Evite a "inflação de estilo de vida" ao ganhar mais.',
    en: 'Avoid "lifestyle inflation" when you earn more.',
    es: 'Evita la "inflación de estilo de vida" al ganar más.',
  },
  {
    pt: 'Mantenha controle de pequenos gastos diários: eles somam.',
    en: 'Track small daily expenses: they add up.',
    es: 'Manten el control de pequeños gastos diarios: se suman.',
  },
  {
    pt: 'Prefira garantia estendida apenas em itens caros e frágeis.',
    en: 'Only buy extended warranty on pricey, fragile items.',
    es: 'Prefiere garantía extendida solo en artículos caros y frágiles.',
  },
  {
    pt: 'Use aplicativos de cashback em combústivel e mercado.',
    en: 'Use cashback apps for fuel and groceries.',
    es: 'Usa apps de cashback en combustible y supermercado.',
  },
  {
    pt: 'Não adianta ganhar mais se os gastos crescem na mesma proporção.',
    en: 'Earning more helps little if spending grows the same.',
    es: 'No sirve ganar más si los gastos crecen en la misma proporción.',
  },
  {
    pt: 'Crie um fundo para "sonhos": viagem, curso, negócio.',
    en: 'Build a "dream fund" for trips, courses, business.',
    es: 'Crea un fundo para "sueños": viaje, curso, negocio.',
  },
  {
    pt: 'Evite compras em grupo quando for só por pressão social.',
    en: 'Avoid group buys driven only by social pressure.',
    es: 'Evita compras en grupo solo por presión social.',
  },
  {
    pt: 'Reveja sua carteira de seguros uma vez por ano.',
    en: 'Review your insurance portfolio once a year.',
    es: 'Revisa tu cartera de seguros una vez al año.',
  },
  {
    pt: 'Use dinheiro para dias de "dieta financeira" controlada.',
    en: 'Use cash on controlled "financial diet" days.',
    es: 'Usa efectivo en días de "dieta financiera" controlada.',
  },
  {
    pt: 'Pague o cartão todo mês: juros de rotação são caríssimos.',
    en: 'Pay the card in full monthly: revolving interest is costly.',
    es: 'Paga la tarjeta todos los meses: los intereses de rotación son carísimos.',
  },
  {
    pt: 'Tenha metas de "não gastos" tanto quanto de gastos.',
    en: 'Set "no-spend" goals as much as spend goals.',
    es: 'Ten metas de "no gastar" tanto como de gastar.',
  },
  {
    pt: 'Evite lojas de atacado se mora sozinho e o alimento estraga.',
    en: 'Skip bulk stores if you live alone and food spoils.',
    es: 'Evita tiendas de mayoreo si vives solo y la comida se echa a perder.',
  },
  {
    pt: 'Aprenda a dizer "não" para convites que furam o orçamento.',
    en: 'Learn to say "no" to invites that break the budget.',
    es: 'Aprende a decir "no" a invitaciones que rompen el presupuesto.',
  },
  {
    pt: 'Use a regra 10-10-10: impacto em 10 dias, 10 meses, 10 anos.',
    en: 'Use the 10-10-10 rule: impact in 10 days, 10 months, 10 years.',
    es: 'Usa la regla 10-10-10: impacto en 10 días, 10 meses, 10 años.',
  },
  {
    pt: 'Mantenha um "diário de gratidão" para comprar menos por emoção.',
    en: 'Keep a gratitude journal to buy less on emotion.',
    es: 'Manten un "diario de gratitud" para comprar menos por emoción.',
  },
  {
    pt: 'Compre livros e cursos: conhecimento rende juros altos.',
    en: 'Buy books and courses: knowledge pays high returns.',
    es: 'Compra libros y cursos: el conocimiento paga intereses altos.',
  },
  {
    pt: 'Não use "parceria" para justificar gastos supérfluos.',
    en: 'Do not use "sharing" to justify needless spending.',
    es: 'No uses la "compañía" para justificar gastos supérfluos.',
  },
  {
    pt: 'Estabeleça um teto de gastos com cartão por mês.',
    en: 'Set a monthly card-spending ceiling.',
    es: 'Establece un tope de gasto con tarjeta por mes.',
  },
  {
    pt: 'Use a técnica "compre um, use um ano" antes de repetir.',
    en: 'Use the "buy one, use one year" test before repeating.',
    es: 'Usa la técnica "compra uno, usa un año" antes de repetir.',
  },
  {
    pt: 'Evite juros compostos contra você: quite cedo.',
    en: 'Avoid compound interest against you: pay off early.',
    es: 'Evita intereses compuestos en su contra: paga temprano.',
  },
  {
    pt: 'Mantenha foco no "porquê" do seu plano financeiro.',
    en: 'Keep focus on the "why" of your financial plan.',
    es: 'Manten el foco en el "porqué" de tu plan financiero.',
  },

  // === Dicas de autores famosos de finanças (citação explícita) ===
  {
    autor: 'Rick Chester',
    pt: 'Rick Chester ensina a Regra de 3: separe todo salário ou pagamento recebido em 3 partes iguais — 1/3 para o seu salário, 1/3 para um fundo de emergência e 1/3 para reinvestir no seu próprio negócio.',
    en: 'Rick Chester teaches the Rule of 3: split every salary or payment received into 3 equal parts — 1/3 for your pay, 1/3 for an emergency fund, and 1/3 to reinvest in your own business.',
    es: 'Rick Chester enseña la Regla de 3: divide cada salario o pago recibido en 3 partes iguales — 1/3 para tu sueldo, 1/3 para un fondo de emergencia y 1/3 para reinvertir en tu propio negocio.',
  },
  {
    autor: 'Warren Buffett',
    pt: 'Warren Buffett: "O preço é o que você paga; o valor é o que você recebe." Não confunda o que custa com o que vale.',
    en: 'Warren Buffett: "Price is what you pay; value is what you get." Do not confuse what costs with what is worth.',
    es: 'Warren Buffett: "El precio es lo que pagas; el valor es lo que recibes." No confundas lo que cuesta con lo que vale.',
  },
  {
    autor: 'Warren Buffett',
    pt: 'Warren Buffett: "Não coloque todos os ovos na mesma cesta." Diversifique para proteger seu patrimônio.',
    en: 'Warren Buffett: "Do not put all your eggs in one basket." Diversify to protect your wealth.',
    es: 'Warren Buffett: "No pongas todos los huevos en la misma cesta." Diversifica para proteger tu patrimonio.',
  },
  {
    autor: 'Benjamin Franklin',
    pt: 'Benjamin Franklin: "Um tostão economizado é um tostão ganho." Pequenas economias se acumulam.',
    en: 'Benjamin Franklin: "A penny saved is a penny earned." Small savings add up.',
    es: 'Benjamin Franklin: "Un centavo ahorrado es un centavo ganado." Los pequeños ahorros se acumulan.',
  },
  {
    autor: 'George S. Clason',
    pt: 'George S. Clason (O Homem Mais Rico da Babilônia): "Parte de tudo que ganhas deve ser tua para guardar." Pague a si mesmo primeiro.',
    en: 'George S. Clason (The Richest Man in Babylon): "A part of all you earn is yours to keep." Pay yourself first.',
    es: 'George S. Clason (El Hombre Más Rico de Babilonia): "Una parte de todo lo que ganas debe ser tuya para guardar." Páguese a sí mismo primero.',
  },
  {
    autor: 'Robert Kiyosaki',
    pt: 'Robert Kiyosaki: "Os ricos não trabalham pelo dinheiro; fazem o dinheiro trabalhar por eles." Compre ativos, não passivos.',
    en: 'Robert Kiyosaki: "The rich do not work for money; they make money work for them." Buy assets, not liabilities.',
    es: 'Robert Kiyosaki: "Los ricos no trabajan por dinero; hacen que el dinero trabaje por ellos." Compra activos, no pasivos.',
  },
  {
    autor: 'Dave Ramsey',
    pt: 'Dave Ramsey: "Viva como ninguém vive hoje, para poder viver como ninguém viverá amanhã." Sacrifique o presente pela liberdade futura.',
    en: 'Dave Ramsey: "Live like no one else today, so you can live like no one else tomorrow." Sacrifice the present for future freedom.',
    es: 'Dave Ramsey: "Vive como nadie vive hoy, para poder vivir como nadie vivirá mañana." Sacrifica el presente por la libertad futura.',
  },
  {
    autor: 'Charlie Munger',
    pt: 'Charlie Munger: "O primeiro dever da inteligência é evitar a estupidez, não o saber tudo." Evite erros caros antes de buscar ganhos.',
    en: 'Charlie Munger: "The first duty of intelligence is to avoid stupidity, not to learn everything." Avoid costly mistakes before chasing gains.',
    es: 'Charlie Munger: "El primer deber de la inteligencia es evitar la estupidez, no saberlo todo." Evita errores costosos antes de buscar ganancias.',
  },
  {
    autor: 'Suze Orman',
    pt: 'Suze Orman: "Quando você tem controle do seu dinheiro, tem controle da sua vida." Organização financeira é autonomia.',
    en: 'Suze Orman: "When you control your money, you control your life." Financial organization is autonomy.',
    es: 'Suze Orman: "Cuando controlas tu dinero, controlas tu vida." La organización financiera es autonomía.',
  },
  {
    autor: 'Thiago Nigro (Primo Rico)',
    pt: 'Thiago Nigro: "O segredo não é ganhar muito, é não perder." Proteja o que já conquistou.',
    en: 'Thiago Nigro (Primo Rico): "The secret is not to earn a lot, but not to lose." Protect what you have already conquered.',
    es: 'Thiago Nigro (Primo Rico): "El secreto no es ganar mucho, es no perder." Protege lo que ya conquistaste.',
  },
];
...
globalThis.totalPago = function (d) {
  return _domTotalPago(d, estado.pagamentos);
};
globalThis.saldoDivida = function (d) {
  return _domSaldoDivida(d, estado.pagamentos);
};
...
globalThis.resumoParcelas = function (d) {
  return _domResumoParcelas(d, estado.pagamentos);
};
globalThis.sincronizarParcela = function (divida, parcelaId) {
  return _domSincronizarParcela(divida, parcelaId, estado.pagamentos);
};
...
function atualizarSelectsNotificacoes() {
  ...
  document.querySelectorAll('[data-notif-intervalo]').forEach((sel) => {
    sel.value = val;
  });
...
function renderDropdownFrequenciaHTML(valor) {
  ...
  const itens = intervalos
    .map((min) => {
      const on = min === v;
      return (
        `<button type="button" class="notif-dd-item${on ? ' active' : ''}" role="option" ` +
...
function t(k) {
  return I18N[idiomaAtual] && I18N[idiomaAtual][k] != null
    ? I18N[idiomaAtual][k]
    : I18N.pt[k] != null
      ? I18N.pt[k]
      : k;
...
function aplicarTema() {
  ...
  if (window.ChartGraficos && window.ChartGraficos.atualizarCores) {
    try {
      window.ChartGraficos.atualizarCores();
    } catch (_) {}
  }
}
function aplicarIdioma() {
  ...
  try {
    localStorage.setItem('appIdioma', idiomaAtual);
  } catch (e) {}
...
function traduzirEstaticos() {
  ...
  const map = {
    exportar: 'acao.exportar',
    importar: 'acao.importar',
...
function atualizarGearCripto() {
  ...
  if (gearCripto) {
    const ativa = !!(
      ...
      estado.configuracoes.criptografia &&
      estado.configuracoes.criptografia.ativa
    );
...
function atualizarBadges() {
  ...
  const pendentes = estado.dividas.reduce(
  ...
  const setBadge = (id, valor, alerta = false) => {
    ...
    if (!valor) {
      el.hidden = true;
      el.textContent = '';
      el.classList.remove('nav-badge--alerta');
      return;
    }
    el.hidden = false;
  ...
  setBadge(
    'badge-lixeira',
    estado.lixeira.dividas.length +
      estado.lixeira.carteiras.length +
      estado.lixeira.recorrentes.length +
      estado.lixeira.metas.length
  );
...
function calcularMetricas() {
  ...
  const totalPago = estado.pagamentos.reduce(
    (acc, p) => somaDinheiro(acc, numDinheiro(p.valor)),
    0
  );
  ...
  const porStatus = Object.keys(todosStatus)
    ...
    .map((k) => ({
      key: k,
      label: t(STATUS_LABEL[k]) || k,
      cor: CORES_STATUS[k] || '#64748b',
      qtd: todosStatus[k],
    }));
  ...
  return {
    ...
    porCategoria,
    porStatus,
    atrasadasDividas: atrasadasDividas.size,
  };
}
...
function gerarInsights(m) {
  ...
  if (m.progresso >= 75)
    out.push({
      tipo: 'bom',
      ico: ICON.festa,
      texto: ti('insight.quitadoAlto', { p: m.progresso.toFixed(0) }),
    });
  else if (m.progresso >= 40)
    out.push({
      tipo: 'bom',
      ico: ICON.forca,
      texto: ti('insight.quitadoMedio', { p: m.progresso.toFixed(0) }),
    });
  else if (m.progresso > 0)
    out.push({
      tipo: 'aten',
      ico: ICON.ampulheta,
      texto: ti('insight.quitadoBaixo', { p: m.progresso.toFixed(0) }),
    });
  else out.push({ tipo: 'ruim', ico: ICON.alerta, texto: ti('insight.nenhumPagamento') });

  const atrasadas = m.atrasadasDividas || 0;
  if (atrasadas > 0)
    out.push({ tipo: 'ruim', ico: ICON.alerta, texto: ti('insight.atrasadas', { n: atrasadas }) });

  const maior = estado.dividas.reduce(
    (max, d) => (totalDivida(d) > totalDivida(max) ? d : max),
    estado.dividas[0]
  );
  if (maior)
    out.push({
      tipo: 'neutro',
      ico: ICON.marcador,
      texto: ti('insight.maior', {
        d: escapeHtml(maior.descricao),
        v: fmt.format(totalDivida(maior)),
      }),
    });
  ...
  if (cartao > 0)
    out.push({
      tipo: 'aten',
      ico: ICON.cartao,
      texto: ti('insight.cartao', { p: ((cartao / m.totalGeral) * 100).toFixed(0) }),
    });

  const negociadas = (m.porStatus.find((s) => s.key === 'negociado') || {}).qtd || 0;
  if (negociadas > 0)
    out.push({ tipo: 'bom', ico: ICON.acordo, texto: ti('insight.negociadas', { n: negociadas }) });
  ...
  if (totalParcelas > 12)
    out.push({
      tipo: 'aten',
      ico: ICON.quebra,
      texto: ti('insight.muitasParcelas', { n: totalParcelas }),
    });
...
async function carregar() {
  ...
  try {
    const lp = await listarPerfis();
    perfilLogadoId = (lp && lp.ativo) || '';
  } catch (_) {
    perfilLogadoId = '';
  }
  if (!estado.configuracoes) estado.configuracoes = { moeda: 'BRL' };
  if (typeof estado.configuracoes.notificarVencimento !== 'boolean')
    estado.configuracoes.notificarVencimento = true;
  if (typeof estado.configuracoes.antecedenciaNotif !== 'number')
    estado.configuracoes.antecedenciaNotif = 3;
  ...
  if (cfg.idioma && cfg.idioma !== idiomaAtual) {
    idiomaAtual = cfg.idioma;
    aplicarIdioma();
  }
  ...
  if (cfg.acento && cfg.acento !== acentoAtual) {
    acentoAtual = cfg.acento;
    aplicarAceno();
  }
...
function abrirModalDesbloqueio() {
  ...
  const entrarSemSenha = () => {
    ...
    if (window.mostrarToast)
      window.mostrarToast(
        t('cripto.semSenhaAviso') || 'Dados ocultos. Digite a senha depois para acessá-los.',
        'info'
      );
    if (window.render) window.render();
  };
  abrirModal(
    ...
    async (vals) => {
      ...
      if (!senha) {
        if (window.mostrarToast)
          window.mostrarToast(
            t('cripto.senhaVazia') || 'Digite a senha para desbloquear os dados.',
            'erro'
          );
      ...
      if (!r || !r.ok) {
        if (window.mostrarToast)
          window.mostrarToast(
            t('cripto.senhaIncorreta') || 'Senha incorreta. Tente novamente.',
            'erro'
          );
        return false; // mantem o modal aberto
      }
      if (window.mostrarToast)
        window.mostrarToast(t('cripto.desbloqueado') || 'Dados desbloqueados', 'sucesso');
      await carregar();
...
async function listarPerfis() {
  try {
    return await window.api.perfilListar();
  } catch (_) {
...
async function abrirSelecaoPerfil() {
  ...
  const ativoId =
    logado && perfilLogadoId ? perfilLogadoId : logado && ativoMem ? ativoMem : ativo || '';
  ...
  const itens =
    ...
    <div class="perfil-item">
      ...
      .join('') || `<p class="modal-msg">${t('perfil.nenhum')}</p>`;
  abrirModal(t('perfil.selecione') || 'Selecionar perfil', [], () => true, {
...
async function trocarPerfil(id) {
  ...
  if (!r || !r.ok) {
    if (window.mostrarToast)
      window.mostrarToast(t('perfil.erroTrocar') || 'Não foi possível trocar o perfil', 'erro');
    return;
  }
...
function criarPerfilFlow() {
  abrirModal(
    t('perfil.criar') || 'Criar novo perfil',
    [
      {
        label: t('perfil.nome') || 'Nome do perfil',
        name: 'nome',
        type: 'text',
        required: true,
        placeholder: t('perfil.nomePlaceholder') || 'Ex.: Esposa',
      },
      {
        label: t('perfil.criptografar') || 'Criptografar este perfil?',
        name: 'cripto',
        type: 'checkbox',
      },
      {
        label: t('cripto.definirSenha') || 'Senha',
        name: 'senha',
        type: 'password',
        required: false,
      },
      {
        label: t('cripto.confirmarSenha') || 'Confirmar senha',
        name: 'senhaConfirmar',
        type: 'password',
        required: false,
      },
    ...
    async (vals) => {
      ...
      if (!nome) {
        if (window.mostrarToast)
          window.mostrarToast(t('perfil.nomeVazio') || 'Informe um nome', 'erro');
        return false;
      }
      if (vals.cripto && vals.senha) {
        if (vals.senha !== vals.senhaConfirmar) {
          if (window.mostrarToast)
            window.mostrarToast(
              t('cripto.senhasDiferem') || 'As senhas não conferem. Digite novamente.',
              'erro'
            );
          return false;
        }
      ...
      if (!r || !r.ok) {
        if (window.mostrarToast)
          window.mostrarToast(t('perfil.erroCriar') || 'Não foi possível criar o perfil', 'erro');
        return false;
      }
      ...
      if (vals.cripto && vals.senha) {
        ...
        if (!ra || !ra.ok) {
          if (window.mostrarToast) window.mostrarToast(t('cripto.erro'), 'erro');
        } else {
      ...
      if (window.mostrarToast)
        window.mostrarToast(t('perfil.criado') || 'Perfil criado', 'sucesso');
      return true;
    }
  );
}
...
function gerenciarPerfil(id) {
  ...
  abrirModal(
    ...
    [
      {
        label: t('perfil.novoNome') || 'Novo nome do perfil',
        name: 'nome',
        type: 'text',
        required: true,
        value: p.nome || '',
      ...
      {
        label: t('cripto.novaSenha') || 'Nova senha',
        name: 'senhaNova',
        type: 'password',
        required: false,
      },
    ...
    async (vals) => {
      ...
      if (vals.senhaNova) {
        const rt = await window.api.perfilTrocarSenha({
          id,
          senhaAtual: vals.senhaAtual || '',
          senhaNova: vals.senhaNova,
        });
        if (!rt || !rt.ok) {
          if (window.mostrarToast)
            window.mostrarToast(
              (t('perfil.erroSenha') || 'Falha ao trocar senha') + ': ' + ((rt && rt.erro) || ''),
              'erro'
            );
          return false;
        }
...
function ativarCriptografia() {
  abrirModal(
    t('cripto.ativar') || 'Ativar criptografia',
    [
      {
        label: t('cripto.definirSenha') || 'Defina uma senha para criptografar os dados:',
        name: 'senha',
        type: 'password',
        required: true,
      },
    ...
    async (vals) => {
      ...
      if (senha !== (vals && vals.senhaConfirmar)) {
        if (window.mostrarToast)
          window.mostrarToast(
            t('cripto.senhasDiferem') || 'As senhas não conferem. Digite novamente.',
            'erro'
      ...
      if (r && r.ok) {
        ...
        if (window.render) window.render();
        if (window.mostrarToast)
          window.mostrarToast(t('cripto.ativada') || 'Criptografia ativada', 'sucesso');
        return true;
      }
      if (window.mostrarToast)
        window.mostrarToast((t('cripto.erro') || 'Erro') + ': ' + ((r && r.erro) || ''), 'erro');
      return false;
    }
  );
}
...
async function desativarCriptografia() {
  ...
  if (r && r.ok) {
    ...
    if (window.render) window.render();
    if (window.mostrarToast)
      window.mostrarToast(t('cripto.desativada') || 'Criptografia desativada', 'sucesso');
...
async function verificarAtualizacaoManual() {
  if (!window.api || typeof window.api.updateVerificarAgora !== 'function') {
    if (window.mostrarToast)
      window.mostrarToast(t('upd.verificando') || 'Verificando atualizações…', 'info');
    return;
  }
  if (window.mostrarToast)
    window.mostrarToast(t('upd.verificando') || 'Verificando atualizações…', 'info');
  try {
    ...
    if (window.mostrarToast)
      window.mostrarToast(
        (t('upd.erro') || 'Erro') + ': ' + (e && e.message ? e.message : e),
        'erro'
      );
  }
}
...
async function migrarXPgestao() {
  ...
  //    e obter níveis não-decrescentes no tempo, iteramos DE TRÁS PARA FRENTE.
  {
    ...
    for (let i = g.historico.length - 1; i >= 0; i--) {
      ...
      if (h.nivel !== nivelCorreto) {
        h.nivel = nivelCorreto;
        alterou = true;
      }
    }
    // Remove a flag legada para não deixar rastro de estado inconsistente.
    if (g.niveisHistoricoCorrigido) {
      delete g.niveisHistoricoCorrigido;
    }
  }
...
const MAPA_TEXTO_CHAVE = (() => {
  ...
  for (const chave in MAPA_MOTIVO) {
    for (const lang of ['pt', 'en', 'es']) {
      const txt =
        I18N[lang] && I18N[lang][chave] != null
          ? I18N[lang][chave]
          : I18N.pt[chave] != null
            ? I18N.pt[chave]
            : chave;
...
function ganharXP(pontos, motivo) {
  ...
  if (estado.gamificacao.nivel > antes) {
    ...
    try {
      ...
      console.error('Falha na celebração de nível:', e);
    }
  }
}
...
function pararConfete() {
  if (_levelupRaf) {
    cancelAnimationFrame(_levelupRaf);
    _levelupRaf = null;
  }
  const canvas = document.getElementById('levelup-confetti');
  if (canvas) {
...
document.addEventListener('click', (e) => {
  if (e.target && e.target.closest('[data-acao="fechar-levelup"]')) {
    fecharLevelUp();
    return;
  }
  const overlay = document.getElementById('levelup-overlay');
  if (
    overlay &&
    overlay.classList.contains('visivel') &&
    e.target === overlay &&
    !e.target.closest('.levelup-card')
  ) {
    fecharLevelUp();
  }
});
...
function atualizarBadgeNivel() {
  ...
  el.innerHTML =
    `<div class='perfil-texto'><span class='nivel-ico'>${ICON.trofeu}</span> ${t('nivel.titulo')} ${nivel} · ${tituloNivel(nivel)}</div>` +
...
function parcelasParaFormulario(n, parcelasExistentes = []) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const opts = STATUS_OPTIONS.map(
      (o) =>
        `<option value="${o.value}"${o.value === (existente?.status || 'pendente') ? ' selected' : ''}>${t('status.' + o.value)}</option>`
    ).join('');
...
function novaDivida() {
  abrirModal(
    t('divida.nova').replace(/^\+\s*/, '') || 'Nova dívida',
    [
      {
        name: 'descricao',
        label: t('form.descricao'),
        type: 'text',
        placeholder: t('form.exDescricao'),
        required: true,
      },
      {
        name: 'credor',
      ...
      {
        name: 'numParcelas',
        label: t('form.numero') + ' de parcelas',
        type: 'number',
        step: '1',
        min: '1',
        placeholder: '1',
  ...
  abrirModal(
    ...
    [
      ...
      },
      {
        ...
        min: '1',
        placeholder: '1',
        value: String(parcelasAtuais.length || 1),
        required: true,
        id: 'num-parcelas-input',
      },
      {
        name: 'observacao',
        label: t('form.observacao'),
        type: 'textarea',
        value: d.observacao || '',
      },
    ],
...
async function esvaziarLixeiraTudo() {
  const total =
    estado.lixeira.dividas.length +
    estado.lixeira.pagamentos.length +
    estado.lixeira.carteiras.length +
    estado.lixeira.recorrentes.length +
    estado.lixeira.metas.length;
  if (total === 0) {
    toast(t('lixeira.vazia'));
    return;
  }
  abrirConfirmacao({
...
async function restaurarCarteira(id) {
  restaurarItemLixeira('carteiras', id, t('toast.carteiraRestaurada'));
}
async function restaurarRecorrente(id) {
  restaurarItemLixeira('recorrentes', id, t('toast.recorrenteRestaurado'));
}
async function restaurarMeta(id) {
  restaurarItemLixeira('metas', id, t('toast.metaRestaurada'));
}
...
function novoPagamento(dividaPreSelecionada = null) {
  ...
  const pagasSet = new Set(
    estado.pagamentos
      .filter((p) => p.dividaId === dividaPreSelecionada?.id && p.parcelaId)
      .map((p) => p.parcelaId)
  );
  ...
  abrirModal(
    t('modal.registrarPagamento'),
    [
      {
        name: 'dividaId',
        label: t('form.divida'),
        type: 'select',
        value: preSelecDiv?.id || opcoes[0].value,
        options: opcoes,
      },
      {
        name: 'parcelaId',
        label: t('pagamento.parcela'),
        type: 'select',
        value: parcelaInicial,
        options: opcoesParcela(preSelecDiv),
      },
      {
        name: 'valor',
      ...
      { name: 'data', label: t('form.dataPagamento'), type: 'date', value: hoje(), required: true },
      {
        ...
        value: preSelecDiv?.observacao || '',
      },
      {
        name: 'carteiraId',
        label: t('form.carteira'),
        type: 'select',
        value: '',
        options: [{ value: '', label: t('nenhuma') || '—' }].concat(
          (estado.carteiras || []).map((c) => ({ value: c.id, label: c.nome }))
        ),
      },
    ],
    async (v) => {
      ...
      if (!r.ok) {
        render();
        return;
      } // usuário cancelou saldo insuficiente
  ...
  const atualizarResumoParcela = () => {
    ...
    resumoEl.innerHTML =
      resumoParcelasHtml(d, r) + (selP ? pagamentosParcelaHtml(d, selP.value) : '');
  };
...
function editarPagamento(p) {
  ...
  abrirModal(
    t('modal.editarPagamento'),
    [
      {
        name: 'dividaId',
        label: t('form.divida'),
        type: 'select',
        value: pagamento.dividaId,
        options: estado.dividas.map((d) => ({
          value: d.id,
          label: `${d.descricao} — ${fmt.format(saldoDivida(d))}`,
        })),
      },
      {
        ...
        label: t('pagamento.parcela'),
        type: 'select',
        value: pagamento.parcelaId || '',
        options: opcoesParcela(divida),
      },
      {
        name: 'valor',
        label: t('form.valorPago') + ' (' + t('moeda') + ')',
        ...
        inputmode: 'decimal',
        step: '0.01',
        value: String(Number(pagamento.valor) || 0),
        required: true,
      },
      {
        name: 'data',
        label: t('form.dataPagamento'),
        type: 'date',
        value: pagamento.data || hoje(),
        required: true,
      },
      {
        name: 'nota',
    ...
    async (v) => {
      ...
      if (!pertence) {
        const pagas = new Set(
          estado.pagamentos
            .filter((x) => x.dividaId === novaDivida.id && x.parcelaId && x.id !== pagamento.id)
            .map((x) => x.parcelaId)
        );
  ...
  const preencherCamposDaParcela = (parcelaAtual = false) => {
    ...
    const pag = estado.pagamentos.find(
      (x) => x.dividaId === d.id && x.parcelaId === parcelaId && x.id !== pagamento.id
    );
    if (valEl) {
      valEl.value = pag ? String(Number(pag.valor) || 0) : '';
    }
    if (datEl) {
      datEl.value = pag ? pag.data || hoje() : hoje();
    }
    if (notEl) {
      notEl.value = pag ? pag.nota || '' : '';
    }
  };
  ...
  if (resumoEl) {
    ...
    resumoEl.innerHTML =
      resumoParcelasHtml(divida, r) + pagamentosParcelaHtml(divida, pagamento.parcelaId || '');
  }
...
function abrirGestaoDivida(d) {
  ...
  const linhaParcela = (parc) => {
    ...
    const pct =
      (Number(parc.valor) || 0) > 0 ? Math.round((pago / (Number(parc.valor) || 0)) * 100) : 0;
    const pagamentoExistente = estado.pagamentos.find(
      (x) => x.dividaId === d.id && x.parcelaId === parc.id
    );
    const acao = pagamentoExistente ? t('acao.editar') : t('acao.pagar');
    const statusPago =
      pago >= (Number(parc.valor) || 0) && (Number(parc.valor) || 0) > 0
  ...
  document.getElementById('btn-concluir-divida').onclick = () => {
    ...
    const pago = estado.pagamentos
      .filter((p) => p.dividaId === d.id)
      .reduce((a, p) => somaDinheiro(a, numDinheiro(p.valor)), 0);
...
function lancarPagamentoParcela(d, parc) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    toast(
      existente ? t('toast.pagamentoAtualizado') : t('toast.pagamentoParcelaRegistrado'),
      'success'
    );
  ...
  if (primeiro) {
    ...
    setTimeout(() => {
      primeiro.focus();
      if (primeiro.value && primeiro.select) primeiro.select();
    }, 100);
  }
}
...
function render() {
  ...
  if (ativo && ativo.classList && ativo.classList.contains('campo-busca')) {
    try {
      _focoBusca = { start: ativo.selectionStart, end: ativo.selectionEnd, val: ativo.value };
    } catch (_) {}
  }
...
function carteiraPorId(id) {
  return (estado.carteiras || []).find((c) => c.id === id) || null;
}
...
function novaCarteira() {
  abrirModal(
    t('carteira.nova') || t('carteira.titulo'),
    [
      {
        name: 'nome',
        label: t('carteira.nome'),
        type: 'text',
        placeholder: t('carteira.exNome'),
        required: true,
      },
      {
        name: 'saldo',
        label: t('carteira.saldoInicial') + ' (' + t('moeda') + ')',
        type: 'number',
        step: '0.01',
        placeholder: '0,00',
        value: '0',
...
function editarCarteira(c) {
  ...
  abrirModal(
    ...
    [
      ...
      {
        name: 'saldo',
        label: t('carteira.saldo') + ' (' + t('moeda') + ')',
        type: 'number',
        step: '0.01',
        value: String(c.saldo),
        required: true,
      },
    ],
    async (v) => {
...
function novaRecorrente() {
  abrirModal(
    ...
    [
      {
        name: 'descricao',
        label: t('recorrentes.descricao'),
        type: 'text',
        placeholder: t('recorrentes.exDescricao'),
        required: true,
      },
      {
        name: 'categoria',
        label: t('recorrentes.categoria'),
        type: 'select',
        value: 'servico',
        options: [
      ...
      {
        name: 'valor',
        label: t('recorrentes.valor') + ' (' + t('moeda') + ')',
        type: 'number',
        step: '0.01',
        min: '0',
        placeholder: '0,00',
      ...
      {
        name: 'diaVencimento',
        label: t('recorrentes.diaVencimento'),
        type: 'number',
        step: '1',
        min: '1',
        max: '31',
  ...
  abrirModal(
    ...
    [
      ...
      },
      {
        ...
        min: '1',
        max: '31',
        value: String(r.diaVencimento || 1),
      },
      {
        name: 'observacao',
        label: t('recorrentes.observacao'),
        type: 'textarea',
        value: r.observacao || '',
      },
...
function editarJuros(d) {
  if (!d) return;
  abrirModal(
    t('juros.titulo') + ' — ' + d.descricao,
    [
      {
        name: 'taxaMensal',
        label: t('juros.taxaMensal'),
        type: 'number',
        step: '0.01',
        min: '0',
        placeholder: '0',
        value: String(numDinheiro(d.taxaMensal)),
      },
      {
        name: 'prazoMeses',
        label: t('juros.prazoMeses'),
        type: 'number',
        step: '1',
        min: '1',
        placeholder: '12',
        value: String(d.prazoMeses || 12),
      },
    ],
    async (v) => {
...
function novaMeta() {
  abrirModal(
    ...
    [
      {
        name: 'titulo',
        label: t('metas.descricao'),
        type: 'text',
        placeholder: t('metas.exTitulo'),
        required: true,
      },
      {
        name: 'valorAlvo',
        label: t('metas.valorAlvo') + ' (' + t('moeda') + ')',
        type: 'number',
        step: '0.01',
        min: '0',
        placeholder: '0,00',
      ...
      {
        name: 'valorAtual',
        label: t('metas.valorAtual') + ' (' + t('moeda') + ')',
        type: 'number',
        step: '0.01',
        min: '0',
        placeholder: '0,00',
...
function simularQuitacaoHandler() {
  ...
  const r = simularQuitacao(estado.dividas, {
    estrategia,
...
function initTicker() {
  ...
  const umaVolta = ordemTicker
    .map((d) => {
      const texto = d[idiomaAtual] || d.pt || d;
      const citacao = d.autor
        ? `${texto} <span class="ticker-autor">— ${escapeHtml(d.autor)}</span>`
        : escapeHtml(texto);
...
function bloquearSeNaoCarregado() {
  ...
  if (window.mostrarToast)
    window.mostrarToast(
      t('cripto.entrarSemSenhaAviso') || 'Desbloqueie os dados para registrar novas entradas.',
      'info'
    );
...
const handlers = {
  ...
  'gerenciar-perfil-ativo': () => {
    const id = (window.__perfisInfo && window.__perfisInfo.ativo) || null;
    if (id) gerenciarPerfil(id);
  },
...
function gerarCSV() {
  ...
  linhas.push('DÍVIDAS');
  linhas.push(
    [t('col.divida'), t('col.categoria'), t('col.total'), t('col.pago'), t('col.saldo')]
      .map(esc)
      .join(',')
  );
  for (const d of estado.dividas) {
    linhas.push(
      [
        d.descricao,
        t(CATEGORIAS[d.categoria]?.label) || d.categoria,
        totalDivida(d).toFixed(2),
        totalPago(d).toFixed(2),
        saldoDivida(d).toFixed(2),
      ]
        .map(esc)
        .join(',')
    );
  ...
  linhas.push(
    [t('col.divida'), t('col.parcela'), t('col.valor'), t('form.data'), t('form.nota')]
      .map(esc)
      .join(',')
  );
...
async function exportarCSV() {
  const csv = gerarCSV();
  const r = await window.api.exportarCSV(
    csv,
    'meubolso-' + new Date().toISOString().slice(0, 10) + '.csv'
  );
  if (r.cancelado) return;
  if (r.ok) toast(t('toast.exportado') + r.caminho, 'success');
...
async function exportarPDF() {
  const r = await window.api.exportarPDF(
    'meubolso-relatorio-' + new Date().toISOString().slice(0, 10) + '.pdf'
  );
  if (r.cancelado) return;
  if (r.ok) toast(t('toast.exportado') + r.caminho, 'success');
...
async function dispararNotificacoes() {
  const prefs = notifPrefs();
  if (!prefs.ativo) {
    console.log('[NOTIF] desativado nas preferências — nada a disparar.');
    return;
  }
  ...
  if (!window.api || !window.api.notificarNativa) {
    console.warn(
      '[NOTIF] window.api.notificarNativa indisponível no renderer — não foi possível enviar.'
    );
    return;
  }
  for (const m of msgs) {
...
function agendarNotificacoes(delayPrimeiroMs) {
  ...
  const prefs = notifPrefs();
  if (!prefs.ativo) {
    console.log('[NOTIF] agendamento cancelado (desativado).');
    return;
  }
  ...
  const emDev =
    typeof ambienteAtual !== 'undefined' && ambienteAtual && ambienteAtual !== 'producao';
  const ms = emDev
    ? Math.max(15_000, (prefs.intervaloMin || 5) * 1000)
    : Math.max(60_000, (prefs.intervaloMin || 5) * 60_000);
  notifTimer = setInterval(() => {
    if (dadosCarregados) dispararNotificacoes();
  }, ms);
  ...
  setTimeout(() => {
    console.log('[NOTIF] disparo inicial (após login, ' + delay + 'ms)');
    dispararNotificacoes();
  }, delay);
  console.log(
    '[NOTIF] agendado — intervalo',
    ms,
    'ms (emDev=' + emDev + ', ativo=' + prefs.ativo + ', primeiro em ' + delay + 'ms)'
  );
}
...
async function testarNotificacao() {
  ...
  const feedback = (msg, tipo) => {
    if (typeof window !== 'undefined' && window.mostrarToast)
      window.mostrarToast(msg, tipo || 'info');
...
async function initEscalaLargura() {
  try {
    const base = await window.api.larguraBase();
    if (base && base > 0) LARGURA_BASE = base;
  } catch (_) {}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (fonte) {
      ...
      let idx = NIVEIS.reduce(
        (best, n, i) => (Math.abs(n - s) < Math.abs(NIVEIS[best] - s) ? i : best),
        0
      );
      const idxAntes = idx;
      ...
      try {
        localStorage.setItem('appFontScale', s.toFixed(2));
      } catch (e2) {}
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (!emCampo) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (k === 'n') {
          e.preventDefault();
          if (typeof novaDivida === 'function') novaDivida();
          return;
        }
        if (k === 'p') {
          e.preventDefault();
          if (typeof novaPagamento === 'function') novaPagamento();
          return;
        }
        if (k === 'f') {
          e.preventDefault();
