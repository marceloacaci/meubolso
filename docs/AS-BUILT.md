...
>
> Data do levantamento: agosto/2026 · Commit base: `efc3aed` (v1.2.0) → atualizado em 19/ago (v2.1.0)
...
| 22/ago/2026 | **S11 — Segurança de dados**: B6 undo global (Ctrl+Z desfaz última exclusão restaurando da lixeira), B7b retenção de 30 dias na lixeira (purga automática na carga + botão "Limpar expirados"), i18n pt/en/es · suíte Vitest **182/182** | `src/dominio.js` (selecionarUltimaExclusao/purgarLixeiraExpirados/contarLixeiraExpirados), `app.js` (desfazerUltimaExclusao/limparLixeiraExpirados + atalho Ctrl+Z), `views/lixeira.js`, `tests/s11-seguranca.test.js` |

> Status verificado por suíte automatizada: **182/182 testes** Vitest verdes (`npm run test`) em 22/ago/2026 (S1–S11). Auditoria a11y 8/8 WCAG 2.1 AA. Teste de carga `resumoParcelas`×500 dívidas **4,67 ms**. `app.js` reduzido de 3.519 → ~2.6k LOC.
